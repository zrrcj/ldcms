...
namespace DTcms.Web.tools
{
    ...
    {
        private string Uploadurl = System.Configuration.ConfigurationManager.AppSettings["Uploadurl"];
        ...
        private void getuserlist (HttpContext context)
            {
            string id = DTRequest.GetString("id");
            BLL.manager manager = new BLL.manager();
            BLL.dt_manager_info manager_info_bll = new BLL.dt_manager_info();
            var imllist = manager.GetModelList("");
            var json =new  Model.Root();
            json.code = 0;
            json.data = new Model.Data();
            json.msg = "";
            json.data.mine = new Model.Mine();
            json.data.friend = new List<Model.FriendItem>();
            json.data.group = new List<Model.GroupItem>();
            json.data.friend = new BLL.dt_friend().GetFriendItemList(id);
            json.data.group = new BLL.dt_group_info().GetGroupItemList(id);
            json.data.mine.avatar = manager_info_bll.GetModelId(int.Parse(id)).avatar;
            json.data.mine.id = id;
            json.data.mine.sign = "哈哈";
            json.data.mine.username = new BLL.manager().GetModel(int.Parse(id)).user_name;
            json.data.mine.status = "online";
            context.Response.Write(JsonHelper.ObjectToJSON(json));
            return;
            }
        #endregion
        #region 验证扩展字段是否重复============================
