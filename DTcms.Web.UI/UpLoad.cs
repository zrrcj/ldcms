...
namespace DTcms.Web.UI
{
    public class UpLoad
    {
        ...
        /// <param name="isWater">是否打水印</param>
        /// <returns>上传后文件信息</returns>
        public string fileSaveAsIm (HttpPostedFile postedFile, bool isThumbnail, bool isWater,string type)
            {
            try
                {
                string fileExt = Utils.GetFileExt(postedFile.FileName); //文件扩展名，不含“.”
                ...
                if (type=="file")
                    {
                    return "{\"code\": 0, \"msg\": \"\", \"data\": { \"src\": \"" + newFilePath + "\", \"name\": \""+ fileName + "\"}}";
                    }
                else
                    {
                    return "{\"code\": 0, \"msg\": \"\", \"data\": { \"src\": \"" + newFilePath + "\"}}";
                    }
               
                }
            catch
                {
                return "{\"code\": 0, \"msg\": \"上传过程中发生意外错误！\"}";
                }
            }
        /// <summary>
        /// 保存远程文件到本地
